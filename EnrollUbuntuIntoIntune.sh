...
CHOICE=$(zenity --list --title="Enroll Ubuntu 24.04 to Intune " --text "Install Edge first, then choose Microsoft Intune" --column "Menu" \
...
if [ $? -eq 1 ]; then
    ...
    "Intune - Offboarding")
        ...
        if is_installed "intune-portal"; then
			...
			# cd ~/.config/intune
			sudo rm ~/.config/intune/registration.toml
